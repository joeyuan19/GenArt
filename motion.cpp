...
bool init_sdl() {
    ...
    if ( SDL_Init( SDL_INIT_VIDEO ) < 0 ) {
        ...
        window = SDL_CreateWindow("Motion", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
        if ( window == NULL ) {
...
int f(double x, double y) {
    return 150120*abs(sin(x));
}

int f1(double x, double y) {
    return 1000000*abs(cos(x));
}
